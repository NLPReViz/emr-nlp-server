...
import io.github.nlpreviz.server.model.FeedbackSpan_Model;
 ...
 */
public class TextFileFeedbackManager extends FeedbackManager {
...
//    List<List<Map.Entry<String, Double>>> predictorsSortedTermWeightMap;
	 ...
	 */
	public TextFileFeedbackManager(String feedbackFileName,
...
//            varID = feedback.getNLPVariable().toString();
    ...
    public static FeedbackType getFeedbackType(String[] feedbackLine) {
        FeedbackType feedbackType;
        if(feedbackLine.length == 11) {
            feedbackType = FeedbackType.VariableValue;
        }
        else {
            feedbackType = FeedbackType.HighlightSpan;
        }
    ...
    public void createLearningFiles() throws Exception {
        ...
        for(String[] currentSession : currentSessionList) {
            if(currentSession[4].equals("valid")) {
                createLearningFiles(currentSession[0], currentSession[1], currentSession[2]);
            }
        }
    }
    
    /**
     * Create learning files for a session of a user at a classification task (variable).
     * <p>
     * Save the learning files (if not exists) for the session of a user at the 
     * classification task (variable). And return the Instances objects
     * <p>
     * This implementation is target particularly at Weka learning file.
     * 
     * @param sessionID
     * @param userID
     * @param varID
     * @return An array of Instances, the first element is instance dataset, 
     * the second element is feedback dataset.
     * @throws Exception 
     */
    public Instances[] createLearningFiles(String sessionID, String userID, 
            String varID) throws Exception {
        Instances[] dataSetList = new Instances[2];

        dataSetList[0] = createInstanceSetLearningFile(sessionID, userID, varID);
        dataSetList[1] = createFeedbackSetLearningFile(sessionID, userID, varID);
    ...
    public String getVarValueAtLineID(String lineID) throws Exception {
        ...
        for(int i = 0; i < metaTable.length; i++) {
            if(metaTable[i][0].equals(lineID) &&
                    getFeedbackType(metaTable[i]) == FeedbackType.VariableValue) {
...
//            predictorsKeywordWeightMap.add(predictorsKeywordWeightList);
    ...
    protected String getModelFileName(String sessionID, String userID,
            String varID) throws Exception {
        return getModelFileName(modelFolder, varID, sessionID, userID);
    }
    
    protected String getFeatureWeightFileName(String sessionID, String userID,
            String varID) throws Exception {
