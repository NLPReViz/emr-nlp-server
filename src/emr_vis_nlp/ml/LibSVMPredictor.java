...
import edu.pitt.cs.nih.backend.utils.FeatureWeight;
import edu.pitt.cs.nih.backend.utils.Mergesort;
...
public class LibSVMPredictor extends ALearner {
	...
	/* 
	 * trainSet is a string array where
	 * libSVMPath, fn_trainSet, fn_model, fn_weight
	 * 
	 */
	@Override
	public void train(Object trainSet) throws Exception {
		String[] paramList = (String[]) trainSet;
        String cmdArgs = initialTrainingParameter(paramList[0], paramList[1], paramList[2], paramList[3]);
//        System.out.println(cmdArgs);
	...
	/* 
	 * trainSet is a string array where
	 * libSVMPath, fn_testSet, fn_model, fn_prediction
	 * 
	 */
	@Override
	public double[][] predict(Object testSet) throws Exception {
		String[] paramList = (String[]) testSet;
        String cmdArgs = initialTestingParamter(paramList[0], paramList[1], paramList[2], paramList[3]);
        RunCmdLine.runCommand(cmdArgs, cmdLineVerbolity);
//        System.out.println(cmdArgs);
	...
	public String initialTrainingParameter(String svmLightPath, String _fn_trainSet,
            String _fn_model, String _fn_weight) throws Exception {
        if(Util.fileExists(_fn_trainSet)) {
            fn_trainSet = _fn_trainSet;
        }
        else {
            throw new UnsupportedOperationException("Training set file " + _fn_trainSet +
                    " doesn't exist.");
        }
...
//            cmdLine = "\"" + Util.getOSPath(new String[] {svmLightPath, "svm-train"}) + "\"";
            cmdLine = Util.getOSPath(new String[] {svmLightPath, "svm-train"});
        }
        
        // SVMLight control param
        cmdLine += " -q"; // no output
        cmdLine += " -c 1"; // C = 1 => instance's final Cost = C * instance's Cost (get from train set)
...
//        cmdLine += " " + fn_model; // output model file
    ...
    public String initialTestingParamter(String svmLightPath, String _fn_testSet,
            String _fn_model, String _fn_prediction) throws Exception {
        
        if(Util.fileExists(_fn_testSet)) {
            fn_testSet = _fn_testSet;
        }
        else {
            throw new UnsupportedOperationException("Testing set file " + _fn_testSet +
                    " doesn't exist.");
        }
...
//            cmdLine = "\"" + Util.getOSPath(new String[] {svmLightPath, "svm-predict"}) + "\"";
            cmdLine = Util.getOSPath(new String[] {svmLightPath, "svm-predict"});
        }
...
//        cmdLine += " " + fn_prediction; // output prediction file
	...
	protected double[][] convertLibSVMPredictionIntoPredictionMatrix(
		...
		if (predictionTable[0][0].equals("labels")) {// using probability output
			...
			predictionMatrix = new double[predictionTable.length - 1][2]; // skip the header row
			...
			if(predictionTable[0].length < 3) { // train model with 1 class only
				if(posInd == 1) { // train with positive class only
					...
					// predict 100% on negative class (which is impossible)
					for(int i = 0; i < predictionMatrix.length; i++) {						
				...
				for (int i = 1; i < predictionTable.length; i++) { // skip the header row
					predictionMatrix[i - 1][0] = Double
							.parseDouble(predictionTable[i][negInd]); // make sure it is 0 <=> -1
					predictionMatrix[i - 1][1] = Double
							.parseDouble(predictionTable[i][posInd]); // make sure it is 1 <=> 1
				}
			}
     ...
     * @throws Exception 
     */
    public int[][] getConfusionMatrix(String fn_test, double[][] predictionMatrix)
            throws Exception {
    ...
    /**
     * Get feature weights from the model saved in <code>fn_model</code>.
     * The feature weight order is the feature order.
     * <p>
     * This function is converted from the Python code of Ori Cohen for SVMLight.
     * 
     * @param fn_model
     * @return
     * @throws Exception 
     */
    public FeatureWeight[] getFeatureWeights(String fn_model, boolean sorted) throws Exception {
...
//                comments = lines[lineCount].substring(lines[lineCount].indexOf("#"));
                alpha = Double.parseDouble(features.substring(0, features.indexOf(" ")));
                ...
                for(String p : feat.split(" ")) {
                    featValPair = p.split(":");
                    index = Integer.parseInt(featValPair[0]);
                    w.put(index, w.get(index) + 
                            Double.parseDouble(featValPair[1]) * alpha);
                }                
            }
            ...
            for(Integer featureIndex : w.keySet()) {
                featureWeight[index++] = new FeatureWeight((int) featureIndex, w.get(featureIndex));
            }
            w.clear();
            ...
            if(sorted) { // sort descending
                Mergesort.mergesort(featureWeight, 0, featureWeight.length, false);
            }
        }
    ...
    /**
     * Save feature weight file in the following format featureName, featureWeight 
     * in each row.
     * 
     * @param fn_model
     ...
     * @throws Exception 
     */
    public void saveFeatureWeight(String fn_model, String fn_globalFeatureVector,
            String fn_save, boolean biasFeature) throws Exception {
        ...
        saveFeatureWeight(featureWeight, globalFeatureVector, fn_save, biasFeature);
    }
    
    public void saveFeatureWeight(FeatureWeight[] featureWeight, String[] globalFeatureVector,
            String fn_save, boolean biasFeature) throws Exception {
...
//                continue;
    ...
    public void getMisclassifiedInstanceList(String fn_prediction,
            String fn_testFeatureVector, String fn_testIndex, StringBuilder errorStr) throws Exception {
        ...
        int predictionFileOffset = predictionStrList[0][0].equals("labels") ? 1 : 0;
        for(int i = 0; i < testInstanceList.length; i++) {
            predictValue = Double.parseDouble(predictionStrList[i + predictionFileOffset][0]);
            if(Double.parseDouble(testInstanceList[i].substring(0, 2)) * predictValue < 0) {
                errorStr.append(testInstanceIDTable[i][0]).append(",").append(predictValue).append("\n");
            }
        }
//        errorStr.append(",-------------\n");
    }
